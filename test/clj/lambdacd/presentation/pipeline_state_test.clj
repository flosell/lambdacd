...
(deftest history-test
  (testing "that it converts the internal pipeline-state into a more readable history-representation"
    (testing "that the status will be running while the pipeline is still actiev and that the most recent update will be the pipelines most recent update"
      (is (= [{:build-number 5
               :status :running
    ...
    (testing "that a pipeline will still be in running state while there is a running step and that the earliest start time is the start time of the pipeline"
      (is (= [{:build-number 7
               :status :running
               :most-recent-update-at stop-time
