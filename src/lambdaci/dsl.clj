...
(defn execute-step-new [step args step-id]
...
(defn execute-step-and-merge [args [step-id step]]
  (let [step-result (execute-step-new step args step-id)]
    (merge-step-results args step-result)))


(defn execute-steps-new [steps args step-id]
  (reduce execute-step-and-merge args (steps-with-ids steps step-id)))

(defn in-parallel [& steps]
  (fn [args step-id]
    (execute-steps-new steps args (cons 0 step-id)))) ; FIXME: this isn't real parallelism!

(defn in-cwd [cwd & steps]
  (fn [args step-id]
    (execute-steps-new steps (assoc args :cwd cwd) (cons 0 step-id))))


(defn run [pipeline]
  (let [runnable-pipeline (map eval pipeline)]
    (execute-steps-new runnable-pipeline {} [0])))
